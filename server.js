...
// Database persistence diagnostics endpoint
app.get('/api/debug/persistence-diagnostics', async (req, res) => {
  try {
    console.log('=== MANUAL PERSISTENCE DIAGNOSTICS ===');
    ...
    try {
      ...
      const dirStats = dirExists ? fs.statSync(dbDir) : null;

      diagnostics.tests.directoryAccess = {
        exists: dirExists,
        stats: dirStats ? {
      ...
      // Test write access
      if (dirExists) {
        const testFile = path.join(dbDir, 'api-test.tmp');
        try {
          fs.writeFileSync(testFile, `API test: ${Date.now()}`);
          fs.unlinkSync(testFile);
          diagnostics.tests.directoryAccess.writable = true;
        } catch (writeErr) {
          diagnostics.tests.directoryAccess.writeError = writeErr.message;
        }
      }
    } catch (dirErr) {
      diagnostics.tests.directoryAccess = { error: dirErr.message };
    }

    // Test 2: Database file status
    try {
      ...
      if (dbExists) {
        ...
        diagnostics.tests.databaseFile.stats = {
          size: dbStats.size,
      ...
      diagnostics.tests.databaseFile = { error: dbErr.message };
    }

    // Test 3: Directory contents
    try {
      const dirContents = fs.readdirSync(dbDir);
      diagnostics.tests.directoryContents = {
        files: dirContents,
        count: dirContents.length,
        details: dirContents.map(file => {
      ...
      diagnostics.tests.directoryContents = { error: contentsErr.message };
    }
    ...
    try {
      if (db && db.db) {
        ...
        const queryResult = await new Promise((resolve) => {
          db.db.get("SELECT 1 as test", (err, row) => {
            resolve({ error: err?.message, result: row });
          });
        });

        diagnostics.tests.databaseConnection.canQuery = !queryResult.error;
        diagnostics.tests.databaseConnection.queryResult = queryResult;
      } else {
        diagnostics.tests.databaseConnection = {
          connected: false,
      ...
      diagnostics.tests.databaseConnection = { error: connErr.message };
    }

    res.json(diagnostics);

  } catch (error) {
    console.error('Error running persistence diagnostics:', error);
    res.status(500).json({
      success: false,
      error: error.message,
      timestamp: new Date().toISOString()
